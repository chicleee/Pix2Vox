...
def test_net(cfg,
    ...
    for sample_idx, (rendering_images, ground_truth_volume) in enumerate(test_data_loader):
        ...
        with paddle.no_grad():
            ...
            #       (dt.now(), sample_idx + 1, n_samples, taxonomy_id, sample_name, encoder_loss, refiner_loss, 
    ...
    if test_writer is not None:
        ...
        test_writer.add_scalar(tag='Refiner/EpochLoss', value=refiner_losses.avg, step=epoch_idx)
        test_writer.add_scalar(tag='Refiner/IoU', value=max_iou, step=epoch_idx)
