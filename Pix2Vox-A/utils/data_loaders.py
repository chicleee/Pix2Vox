...
class ShapeNetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print("ShapeNetDataset DatasetLoader")
        # print(taxonomy_name)
        # print(sample_name)

        if self.transforms:
            rendering_images = self.transforms(rendering_images)
        # return taxonomy_name, sample_name, rendering_images, volume #目前paddle不支持str格式的输入，换成数值类型就可以了
...
class ShapeNetDataLoader:
    def __init__(self, cfg):
        ...
        with open(cfg.DATASETS.SHAPENET.TAXONOMY_FILE_PATH, encoding='utf-8') as file:
            ...
            # print("open TAXONOMY_FILE_PATH succeess")
    ...
    def get_dataset(self, dataset_type, n_views_rendering, transforms=None):
        ...
        print('[INFO] %s Complete collecting files of the dataset: ShapeNet. Total files: %d.' % (dt.now(), len(files)))
